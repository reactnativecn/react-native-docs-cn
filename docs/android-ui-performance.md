我们尽我们最大的可能来保证UI组件的性能如丝般顺滑，但有的时候这根本不可能达到。你要记住Android有着超过一万种不同的型号，而在框架底层进行软件渲染的时候是统一处理的，这意味着你没办法像iOS那样自由。不过有些时候，你还是可以想办法提升应用的性能（有的时候问题根本不是出在原生代码上！）

要想解决应用的性能问题，第一步就是搞明白在每个16毫秒的帧中，时间到底用在了什么地方。因此，我们会使用一个标准Android性能分析工具`systrace`，不过在此之前……

> 确定JS的开发者模式已经关闭：
>
> 你应该在你的应用程序日志里看到`__DEV__ === false, development-level warning are OFF, performance optimizations are ON`（你可以通过adb logcat来查看应用程序日志）
...
- `schd`, `gfx`, 和`view`是我们所关心的Android SDK内置的tag（标记的集合）：`schd`提供了你的设备的每个CPU核心正在做什么的信息，`gfx`提供了你的图形相关信息，譬如每帧的时间范围，然后`view`提供了一些关于视图布局和渲染相关性能的信息。
...
打开数据手机的结果后，你应该能看到类似这样的结果：

![Example](../img/SystraceExample.png)

**提示**: 你可以使用WSAD案件来滚动和缩放性能数据图表。
...
滚动图表直到你找到你的应用包名。在上面的例子里，我正在分析`com.facebook.adsmanager`，由于内核讨厌的线程名字长度限制，它会显示成`book.adsmanager`。

在左侧，你应该能看到一系列线程对应着右边的时间轴。有3到4个线程是我们必须关注的：UI线程(名字可能是`UI Thread`或者是你的包名), `mqt_js`和`mqt_native_modules`。如果你在Android 5.0以上版本运行，我们还关心`Render`（渲染）线程。

### UI 线程

标准的Android布局和绘制都在在UI线程里发生。右侧显示的线程名字会是你的包名(在我的例子里是book.adsmanager)或者`UI Thread`.你在这个线程里看到的事件可能会是一些`Choreographer`, `traversals`或者`DispatchUI`：
...
每个背景颜色不同的部分我们称作“一帧”——记住要渲染一个流畅的帧，我们所有的界面工作都需要在16毫秒内完成。注意如果没有任何一个线程在靠近帧的边界处工作。类似这样的一个应用程序就正在60FPS(帧每秒)的情况下流畅表现。

如果你发现一些起伏的地方，譬如这样：

![Choppy Animation from JS](../img/SystraceBadJS.png)

注意在上图中JS线程基本上一直在执行，并且超越了帧的边界。这个应用就没法60FPS的渲染了。在这种情况下，**问题出在JS中**
...
> **TODO**: 我们还在准备更多的JS性能分析的工具，也许会在将来的版本中加入。
...
2. 你在尝试在动画、交互的过程中不断创建新的UI对象（譬如在scroll的过程中加载新的内容）
...
- 检查`renderToHardwareTextureAndroid`的使用，有这个属性的View的子节点正在被动画/变形会导致性能大幅下降(譬如`Navigator`提供的滑动、淡入淡出动画)。
...
如果你还是很迷惑或者不知如何进展，你可以在[Stack Overflow的react-native标签下](http://stackoverflow.com/tags/react-native)提交一个提问。如果你在这里得不到响应，或者找到了一个核心组件的问题，你可以[提交一个Github issue](https://github.com/facebook/react-native/issues)给我们。
