...
<div class="props">
	...
	<div class="prop">
		...
		<div>
			<p>将<code>key</code>字段的值设置成<code>value</code>，并在结束的时候调用<code>callback</code>函数。如果有任何错误发生，则会传递一个<code>Error</code>对象作为第一个参数。返回一个<code>Promise</code>对象。</p>
		</div>
	</div>
	<div class="prop">
	...
	<div class="prop">
		...
		<div>
			<p>将已有的值和新的值合并。假设两个值都是字符串化的JSON。返回一个<code>Promise</code>对象。还没有被所有原生实现都支持。</p>
		</div>
	</div>
	<div class="prop">
		...
		<div>
			<p>删除<em>全部的</em>AsyncStorage数据，不论来自什么库或调用者。通常你都不应该调用这个函数——使用removeItem或者multiRemove来清除你自己的key。返回一个<code>Promise</code>对象。</p>
		</div>
	</div>
	<div class="prop">
	...
	<div class="prop">
		...
		<div>
			<p>获取keys所包含的所有字段的值，调用callback回调函数时传递一个key-value数组的数组。返回一个<code>Promise</code>对象。</p>
	...
	<div class="prop">
		...
		<div>
			<p>删除掉所有键在<code>keys</code>数组中的数据。返回一个<code>Promise</code>对象。</p>
		</div>
	</div>
	<div class="prop">
		...
		<div>
			<p>将输入的值和已有的值合并，假设两个值都是字符串化的JSON。返回一个<code>Promise</code>对象。</p>
			<p>还没有被所有原生实现都支持。</p>
		</div>
	</div>
</div>
